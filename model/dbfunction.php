...
function scheck_user() {
    extract($_POST);
    $con = getConnection();
    $stmt = $con->prepare("select * from supervisor where email = ? and password = ?") or
            die(mysqli_error($con));
    $stmt->bind_param("ss", $eam, $pas);
    $eam = $ea;
    $pas = md5($pa);
...
function getUserRecord() {
    ...
    $stmt->bind_result($id, $name, $casual, $earn, $earns, $vacational_summer, $vacational_winter, $special_casual, $official_duty, $year, $value);
    /* execute query */
    $stmt->execute();
